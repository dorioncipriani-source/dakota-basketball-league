...
const teams = [
  { name: "Minot Outlaws", city: "Minot", slogan: "The Magic City's Most Wanted.", logo: "/images/outlaws.PNG", border: "border-red-800", glow: "shadow-red-950/40" },
  { name: "Bismarck Capitalists", city: "Bismarck", slogan: "Own the Standard.", logo: "/images/capitalists.PNG", border: "border-yellow-600", glow: "shadow-yellow-950/40" },
  { name: "Fargo Reapers", city: "Fargo", slogan: "The Valley Reaps All.", logo: "/images/reapers.PNG", border: "border-purple-700", glow: "shadow-purple-950/40" },
  { name: "Grand Forks Huskies", city: "Grand Forks", slogan: "Forged in Frost.", logo: "/images/huskies.PNG", border: "border-green-700", glow: "shadow-green-950/40" },
  { name: "Williston Roughnecks", city: "Williston", slogan: "Fueled by Grit.", logo: "/images/roughnecks.PNG", border: "border-orange-700", glow: "shadow-orange-950/40" },
];
...
const faqs = [
  ["What is the Dakota Basketball League?", "DBL is a developing competitive men's basketball league built around North Dakota markets, community identity, regional athletes, and sports entertainment."],
  ["Who can play in DBL?", "Players must be 18+, out of high school, and prepared for competitive basketball participation."],
  ["Which cities are included in launch?", "Launch markets currently include Minot, Bismarck, Fargo, Grand Forks, and Williston."],
...
export default function Home() {
  return (
    <main className="min-h-screen overflow-hidden bg-black text-white">
      ...
      <section className="relative flex min-h-[92vh] items-center justify-center border-b border-white/10 px-6 py-24 text-center">
        ...
        <div className="absolute inset-0 opacity-[0.08]">
          <div className="grid h-full grid-cols-5 items-center gap-10 px-8">
            {teams.map((team) => (
              <Image key={team.name} src={team.logo} alt={team.name} width={220} height={220} className="mx-auto h-32 w-auto object-contain md:h-44" />
            ))}
          </div>
        </div>
        ...
        <div className="relative z-10 mx-auto max-w-6xl">
          ...
          <p className="mx-auto mb-10 max-w-3xl text-lg font-medium text-zinc-300 md:text-2xl">
            5 Markets. 1 League. A new competitive basketball platform built for North Dakota athletes, fans, sponsors, and communities.
          </p>
          ...
          <div className="mx-auto grid max-w-3xl grid-cols-3 gap-4">
            <div className="rounded-2xl border border-white/10 bg-white/5 p-5 backdrop-blur">
              ...
              <p className="text-xs uppercase tracking-widest text-zinc-400">Markets</p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/5 p-5 backdrop-blur">
              <p className="text-3xl font-black text-yellow-500">18+</p>
              <p className="text-xs uppercase tracking-widest text-zinc-400">Players</p>
            </div>
            <div className="rounded-2xl border border-white/10 bg-white/5 p-5 backdrop-blur">
              <p className="text-3xl font-black text-yellow-500">2026</p>
              <p className="text-xs uppercase tracking-widest text-zinc-400">Launch</p>
            </div>
          </div>
        </div>
      </section>
        ...
        <div className="mx-auto flex max-w-7xl flex-wrap items-center justify-center gap-8">
          {teams.map((team) => (
            <div key={team.name} className="flex items-center gap-3 opacity-80">
              <Image src={team.logo} alt={team.name} width={46} height={46} className="h-10 w-auto object-contain" />
              <span className="text-sm font-black uppercase tracking-wider text-zinc-300">{team.city}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="teams" className="px-6 py-24">
        <div className="mx-auto max-w-7xl">
          <div className="mb-14 text-center">
            <p className="mb-3 text-sm font-black uppercase tracking-[0.35em] text-red-800">Five Markets. One League.</p>
            <h2 className="text-4xl font-black text-yellow-500 md:text-6xl">Featured Teams</h2>
            <p className="mx-auto mt-5 max-w-2xl text-zinc-400">
              Each DBL franchise represents a distinct Dakota market identity built around local pride, competitive basketball, and community energy.
            </p>
          </div>
      ...
      <section id="sponsorship" className="border-y border-white/10 bg-zinc-950 px-6 py-24">
        <div className="mx-auto max-w-7xl">
          <div className="mb-12 text-center">
            <p className="mb-3 text-sm font-black uppercase tracking-[0.35em] text-red-800">Sponsorship Opportunities</p>
            <h2 className="text-4xl font-black text-yellow-500 md:text-6xl">Build With DBL</h2>
            <p className="mx-auto mt-5 max-w-2xl text-zinc-400">
              Connect your brand to a new regional sports property through league partnerships, team sponsorships, and founding sponsor opportunities.
            </p>
          </div>

          <div className="grid gap-6 md:grid-cols-3">
            {sponsorTiers.map(([name, price]) => (
              <div key={name} className="rounded-3xl border border-white/10 bg-black p-8 transition hover:-translate-y-1 hover:border-yellow-500/50">
                <p className="text-sm font-black uppercase tracking-widest text-red-800">{price}</p>
                <h3 className="mt-3 text-2xl font-black text-white">{name}</h3>
                <p className="mt-4 text-zinc-400">
                  Sponsor the launch of DBL through premium visibility, community alignment, and regional sports branding.
                </p>
              </div>
            ))}
          </div>
      ...
      <section id="players" className="px-6 py-24">
        <div className="mx-auto max-w-7xl rounded-3xl border border-white/10 bg-gradient-to-br from-red-950/50 to-black p-10 text-center md:p-16">
          <p className="mb-3 text-sm font-black uppercase tracking-[0.35em] text-red-700">Player Interest</p>
          <h2 className="text-4xl font-black text-white md:text-6xl">Think You Can Compete?</h2>
          <p className="mx-auto mt-6 max-w-3xl text-lg text-zinc-300">
            DBL is built for competitive men’s basketball players who are 18+, out of high school, and ready to represent their market.
          </p>

          <a href="https://forms.gle/n5BCKUD77MxgBpqH7" target="_blank" className="mt-8 inline-block rounded-lg bg-red-900 px-9 py-4 font-black text-white hover:bg-red-800">
            Complete Interest Form
          </a>
        </div>
      </section>

      <section id="faq" className="border-y border-white/10 bg-black px-6 py-24">
        <div className="mx-auto max-w-6xl">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            {faqs.map(([question, answer]) => (
              <div key={question} className="rounded-3xl border border-white/10 bg-zinc-950 p-7 transition hover:border-yellow-500/40">
      ...
      <section id="news" className="bg-zinc-950 px-6 py-24">
        <div className="mx-auto max-w-7xl">
          <h2 className="mb-12 text-center text-4xl font-black text-yellow-500 md:text-5xl">Latest From DBL</h2>
          ...
          <div className="grid gap-7 md:grid-cols-3">
            {[
              ...
              <article key={headline} className="rounded-3xl border border-white/10 bg-black p-7 transition hover:border-yellow-500/40">
                <p className="mb-3 text-sm font-black uppercase tracking-widest text-red-800">
                  {index === 0 ? "League Update" : index === 1 ? "Sponsorship" : "Player Interest"}
                </p>
                <h3 className="text-2xl font-black">{headline}</h3>
                <p className="mt-4 text-zinc-400">More details coming soon as DBL continues building toward launch.</p>
              </article>
            ))}
          </div>
        </div>
